...
class TimeSeriesPage(QWidget):
    ...
    def _set_table_model(self, df: pd.DataFrame):
        ...
        self.table.table.itemChanged.connect(self._on_table_item_changed)
        self._apply_row_colors()

    def _disconnect_table_signals(self):
        try:
            self.table.table.itemChanged.disconnect(self._on_table_item_changed)
        except Exception:
            pass

    def _on_table_item_changed(self, item):
        if getattr(self.table, "_restoring", False):
            return
        ...
        last_row = self.table.table.rowCount() - 1
        if item.row() == last_row:
            filled = True
            for c in range(self.table.table.columnCount()):
                it = self.table.table.item(last_row, c)
                if it is None or str(it.text()).strip() == "":
                    filled = False
                    break
            if filled and self._pend_row == last_row:
                self._on_row_filled(last_row)
    ...
    def _apply_row_colors(self):
        ...
        if self._input_rows:
            for r in self._input_rows:
                tbl.set_row_color(r, GREEN)
        if self._pend_row is not None:
            tbl.set_row_color(self._pend_row, PEND)
        if self._pred_row is not None:
            tbl.set_row_color(self._pred_row, BLUE)
