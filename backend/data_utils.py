...
VALUE_COLS_TO_DROP = ["值", "XE-133", "CS-137", "KR-89"]
def load_dataset(path: str, log_transform=True):
    """读取 Excel → 加时间特征 → log1p → 标准化"""
    df = pd.read_excel(path)
    df.columns = df.columns.str.strip()
    df[TIME_COL] = pd.to_datetime(df[TIME_COL], format="%Y年%m月%d日%H%M")
    df = df.sort_values(TIME_COL).reset_index(drop=True)
    df['unique_id'] =  'multivariate'
    time =  df[TIME_COL].copy()
    # —— b) 数值特征
    feats = df.drop( VALUE_COLS_TO_DROP, axis=1, errors="ignore")
    feats = feats.apply(pd.to_numeric, errors="coerce").ffill().fillna(0)

    # log1p 可稳定方差
    if log_transform:
        feats = feats.apply(np.log1p)

    scaler = StandardScaler()
    feats_scaled = scaler.fit_transform(feats.astype(np.float32))
    feats[TIME_COL] = time
    feats['unique_id'] =  'multivariate'
    return feats, feats.columns.tolist(), scaler
