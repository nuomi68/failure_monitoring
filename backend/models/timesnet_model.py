...
class TimesNet(nn.Module):
    def __init__(
        ...
        self.out_dim = int(out_dim or num_feat)
        self.head = nn.Conv1d(d_model, self.out_dim, 1)    # 输出回到原特征维
    ...
    def forward(self, x):              # x: (B, L, F)
        ...
        if self.residual:
            last = raw[:, -1, :]  # (B, F)
            if last.shape[1] == x.shape[1]:
                x = x + last
            elif self.target_index is not None:
                x = x + last[:, int(self.target_index)].unsqueeze(1)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if log_callback:
            ...
            if patience > 0 and no_improve >= patience:
                log_callback(
                    f"[TimesNet] early_stop patience={patience} (best_epoch={best_epoch} best_val_loss={best:.4f})"
                )
